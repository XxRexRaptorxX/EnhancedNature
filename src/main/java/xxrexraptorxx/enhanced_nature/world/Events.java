...
import xxrexraptorxx.enhanced_nature.main.EnhancedNature;
...
public class Events {
            ...
            if (!hasShownUp && Minecraft.getInstance().screen == null) {
                ...
                if (modContainer != null) {
                    ...
                    if (versionCheckResult.status() == VersionChecker.Status.OUTDATED || versionCheckResult.status() == VersionChecker.Status.BETA_OUTDATED) {
                        ...
                        EnhancedNature.LOGGER.error(References.NAME + "'s version checker failed!");
                        hasShownUp = true;
                    }
                }
            }
    ...
    private static boolean SupporterCheck(URI uri, Player player) {
        try {
            ...
            EnhancedNature.LOGGER.error("Failed to fetch or process supporter list from URI: {}", uri, e);
            return false;
        }
    }
